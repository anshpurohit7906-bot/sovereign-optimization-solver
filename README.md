...
| Objective difference | ≈ `1.1e-10` (agreement within approx. `1.1e-10`) |
...
| Certificate | `artifacts/pilot87/p87_strict_certificate.txt` |

The certificate is generated by an *independent* KKT check
(`tools/certification/p87_certify.py`) that reconstructs the unscaled
standard-form and original-LP solution from the persisted artifacts and does
**not** trust any objective reported by the solver. See
...
approximately `1.1e-10` (see the [Verified Result](#verified-result-pilot87)).
